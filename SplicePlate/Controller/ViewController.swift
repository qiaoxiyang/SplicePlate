...
class ViewController: UIViewController ,UICollectionViewDelegate,UICollectionViewDataSource,XYSplitViewControllerDelegate{
    ...
    func initSubViews() {
        ...
        widthHeight = self.view.tz_width - 2 * left
        ...
        let splitW = (self.view.tz_width - left - 3 * 2) / 3.0
    ...
    func takePhoto()  {
        ...
        let pickerVC = TZImagePickerController(maxImagesCount: 1, columnNumber: 4, delegate: self)
        pickerVC?.allowPickingVideo = false
        pickerVC?.allowCrop = true
        
        let top = (self.view.tz_height - widthHeight) / 2
        pickerVC?.cropRect = CGRect.init(x: left, y: top, width: widthHeight, height: widthHeight)
